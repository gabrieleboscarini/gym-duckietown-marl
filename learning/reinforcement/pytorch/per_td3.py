...
class TD3(object):
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        sample = replay_buffer.sample(batch_size,  beta=0.4)
        state = torch.FloatTensor(sample["obs"]).to(device)
        action = torch.FloatTensor(sample["action"]).to(device)
        next_state = torch.FloatTensor(sample["next_obs"]).to(device)
        done = torch.FloatTensor(1 - sample["done"]).to(device).reshape(-1,1)
        reward = torch.FloatTensor(sample["reward"]).to(device).reshape(-1,1)
        weights = torch.FloatTensor(sample["weights"]).to(device)
        
        self.beta = min(1.0, self.beta_start + (1.0 - self.beta_start) * (self.total_it / self.beta_frames))
        ...
        #weights = torch.tensor(sample['weights'], dtype=torch.float32).to(device)
        ...
        td_errors = 0.5 * (td_error1.abs() + td_error2.abs())  # shape: [batch_size]
        td_errors = td_errors.detach().cpu().numpy()
        
        #print(sample['indexes'].shape)
        #print(td_errors.shape)

        replay_buffer.update_priorities(sample['indexes'], td_errors + 1e-6)
